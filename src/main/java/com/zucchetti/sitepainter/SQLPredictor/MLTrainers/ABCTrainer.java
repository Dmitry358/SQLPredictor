...
public class ABCTrainer {
    ...
    public void train(String dataTableName, String idField, String classificationField, DataBaseConnecter dbConnecter) {
        //CONNESSIONE CON DB
        //CONTROLLO SE NEL DB NON è GIA PRESENTE TABELLA DI PREDICTOR
        //CONTROLLO SE dataTableName
        String trainQuery = "DROP TABLE IF EXISTS "+this.predictorName+";\n"+
                "CREATE TABLE "+this.predictorName+" AS \n" +
                "SELECT "+idField+", 'A' AS class \n"+
                "FROM (\n" +
                    "SELECT "+idField+", SUM("+classificationField+") OVER (ORDER BY "+classificationField+" desc ) AS run_sum, SUM("+classificationField+") OVER () AS total \n"+
                ...
                "(SELECT sub_A."+idField+", sub_A."+classificationField+" \n" +
                "FROM ( \n" +
                    "SELECT "+idField+", "+classificationField+", SUM("+classificationField+") OVER (ORDER BY "+classificationField+" desc) AS run_sum, SUM("+classificationField+") OVER () AS total \n" +
